...
const oposideSide =  `<svg id="reverse-sort" width="25" height="15" viewBox="0 0 25 15" fill="none"
...
const opositeSort = (type) => {
    if (type == oposideSide) {
        sortingWrapper.innerHTML = sort;
    }
    else if (type == sort) {
        sortingWrapper.innerHTML = oposideSide;
    }
}

const deleteButtons = document.querySelectorAll('.delete');
deleteButtons.forEach((item) => {
    item.addEventListener('click', deletingElements);
});

function deletingElements(e) {
    if (!e.target.matches('.delete')) return;
    e.target.parentElement.parentElement.removeChild(e.target.parentElement);
}
function addingTask() {
    const block = document.querySelector('#block-to-do');
    const addingTask = document.createElement('li');
    addingTask.innerHTML = `${moverSvg}<input class="row-info" type="text">${deleteSvg}`;
    addingTask.className = 'row-to-do';
    addingTask.draggable = 'true';
    block.appendChild(addingTask);
    addingTask.childNodes[2].addEventListener('click', deletingElements);
    dragDrop();
}
